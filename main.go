...
func indexdata(w http.ResponseWriter, r *http.Request) {
	...
	mods := make([]Db, 0, 10)
	mod := Db{}
	rows, err := db.Query("SELECT * FROM xc")
	checkErr(err)
	for rows.Next() {
		var id int64
		var state string
...
func Del(w http.ResponseWriter, r *http.Request) {
	...
	db, err := sql.Open("postgres", "user=postgres password=123456 dbname=test sslmode=disable")
	stm, err := db.Prepare("DELETE  FROM xc WHERE id=$1")
	checkErr(err)
	_, err = stm.Exec(id)
	checkErr(err)
	db.Close()
...
func Updata(w http.ResponseWriter, r *http.Request) {
	...
	id, _ := strconv.ParseInt(idStr, 10, 64)
	db, err := sql.Open("postgres", "user=postgres password=123456 dbname=test sslmode=disable")
	stm, err := db.Prepare("UPDATE xc set state=$1 WHERE id=$2")
	checkErr(err)
...
func Insert(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Print("dasdasd")
	db, err := sql.Open("postgres", "user=postgres password=123456 dbname=test sslmode=disable")
	stm, err := db.Prepare("INSERT INTO xc(state,journey) VALUES($1,$2)")
	checkErr(err)
	_, err = stm.Exec("未完成", xcStr)
	checkErr(err)
	db.Close()
}
